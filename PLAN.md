...
5. **M5 — Dashboard + core UI — DONE.** Dashboard shows real week-over-week revenue/orders/AOV/refund-rate with change indicators plus top open alerts; Insights lists all open alerts by severity; Products/Inventory already read real synced data since M1. Reports intentionally stays a placeholder until M6 (no AI report exists yet). *Demo*: fully navigable app on live store data — confirmed via the production build.
